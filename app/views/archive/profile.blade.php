<?php
  $show = Shows::where('showname', $name)->first();
?>
...
<html lang="en">
  <head>
    ...
    <title>RadioSNHU / Archive for {{ $name }}</title>
      ...
      <div class="row mt centered">
        ...
        <div class="col-md-10 col-md-offset-1">
          ...
          <div class="list-group text-left">
            ...
            @forelse (Archive::where('showid', $show->id)->get() as $arcitem)
              <a href="{{ url('archive/listen/' . $arcitem->arcid) }}" class="list-group-item">
                <h3 class="list-group-item-heading">{{ $arcitem->title }}</h3>
                <p class="list-group-item-text">Uploaded on {{ date('F j, Y', strtotime($arcitem->created_at)) }}</p>
            @empty
              <a href="#" disabled class="list-group-item">There are no archived recordings for this show.</a>
            @endforelse
